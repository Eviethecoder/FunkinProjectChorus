...
class Countdown
{
  ...
  static var fallbackHudStyle:Null<HudStyle>;
   ...
   */
  static function fetchHudStyle(?hudStyleId:String, force:Bool = false):Void
  {
    if (hudStyle != null && !force) return;

    if (hudStyleId == null) hudStyleId = PlayState.instance?.currentChart?.hudStyle;

    hudStyle = HudStyleRegistry.instance.fetchEntry(hudStyleId);
  ...
  public static function showCountdownGraphic(index:CountdownStep):Void
  {
    ...
    var countdownSprite = hudStyle.buildCountdownSprite(index);
    if (countdownSprite == null) return;

    var fadeEase = FlxEase.cubeInOut;
    if (hudStyle.isCountdownSpritePixel(index)) fadeEase = EaseUtil.stepped(8);
    ...
    var offsets = hudStyle.getCountdownSpriteOffsets(index);
  ...
  {
    var path = hudStyle.getCountdownSoundPath(step);
