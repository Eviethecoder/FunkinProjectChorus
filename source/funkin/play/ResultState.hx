...
class ResultState extends MusicBeatSubState
{
  ...
  override function create():Void
  {
    ...
    trace(Preferences.rankingtype);
    switch (Preferences.rankingtype)
    {
      case('Hard'):
        rankDificulty = params.scoreData.tallies.killer;
      case('Intended'):
        rankDificulty = (params.scoreData.tallies.killer + params.scoreData.tallies.sick);
      case('Pussy'):
        rankDificulty = (params.scoreData.tallies.killer + params.scoreData.tallies.sick + params.scoreData.tallies.good);
    }
    add(score);
  ...
  function startRankTallySequence():Void
  {
    ...
    // NOTE: Only divide if totalNotes > 0 to prevent divide-by-zero errors.rankDificulty / params.scoreData.tallies.totalNotes * 100;
    var clearPercentFloat = params.scoreData.tallies.totalNotes == 0 ? 0.0 : rankDificulty / params.scoreData.tallies.totalNotes * 100;
