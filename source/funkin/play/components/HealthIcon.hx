    ...
    if (autoUpdate && PlayState.instance != null)
    {
      switch (playerId)
      {
        case 0: // Boyfriend
          ...
          this.x = PlayState.instance.bars.healthBar.x
            + (PlayState.instance.bars.healthBar.width * (FlxMath.remapToRange(PlayState.instance.bars.healthBar.value, 0, 2, 100, 0) * 0.01)
              - POSITION_OFFSET);
        case 1: // Dad
          // Update the animation based on the current state.
      ...
      this.y = PlayState.instance.bars.healthBar.y - (this.height / 2); // - (PlayState.instance.healthBar.height / 2)
    }
  }
