...
class CharSelectSubState extends MusicBeatSubState
{
  ...
  override function beatHit():Bool
  {
    ...
    if (playerChill.getCurrentAnimation() == "idle")
    {
      playerChill.playAnimation("idle", true, false, false);
    }

    if (gfChill.getCurrentAnimation() == "idle" && (Conductor.instance.currentBeat % gfChill.danceEvery == 0))
    {
      trace('GF beat hit');
